import { Fn, fract, float, floor, mix, sin, vec2, dot, mat2 } from "three/tsl";
...
export const randomGradient = Fn(([p]: [ConstNode<"vec2", Vector2>]) => {
  const x = sin(dot(p, vec2(127.1, 311.7)))
    .mul(43758.5453)
    .fract();
  const y = sin(dot(p, vec2(269.5, 183.3)))
    .mul(43758.5453)
    .fract();
...
export const perlin2D = Fn(
  ([p]: [ConstNode<"vec2", Vector2>]): Node<"float"> => {
    const i = floor(p);
    const f = fract(p);
...
export const fbm = Fn(([st]: [ConstNode<"vec2", Vector2>]) => {
  ...
  // OCTAVE 1
  v.r.assign(v.add(a.mul(perlin2D(p))));
